...
function checkLayoutDocumentation() {
  ...
  const expectedLines = [
    ...
    "이 스크립트는 LAYOUT 핵심 규칙, 상하단 페이지네이션 문자열, 페이지네이션 중복 여부, 작품 홈 목차 순서/중복, 장 제목/부제 블록, 종료 안내 블록 단일성/위치, 장 종료 안내 제목, 장 종료 안내 주인공 언급, 작품 홈 핵심 메타데이터, 배포본 README 핵심 메타데이터, 배포본 README 압축 파일 표 순서/중복, 배포본 README 구성 기준 목록 순서/중복, 목차 링크, 루트 작품 목록 순서/중복, 루트 한 줄 소개 순서/중복, 루트 작품 수/완결 상태, 로컬 링크 파일/앵커, 코드펜스 균형, trailing whitespace, LF line endings, final newline, 배포 zip manifest, zip 내부 원고와 원본의 내용 일치, SHA-256 체크섬을 함께 검사한다.",
    "- `00-prologue.md` — 프롤로그",
