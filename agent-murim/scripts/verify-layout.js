...
function checkNavigation(file) {
  ...
  if (lines[1] !== "" || lines[2] !== "---" || lines[3] !== "") {
    fail(`${rel(file)}: expected top navigation to be followed by blank line, divider, blank line`);
  }
  ...
  if (!last || lines[last.index - 1] !== "" || lines[last.index - 2] !== "---") {
    fail(`${rel(file)}: expected exactly one blank line between bottom divider and bottom navigation`);
  }
}
...
function checkChapterEndBlocks() {
  for (const chapter of chapters) {
    ...
    if (endingBodyLines.some((line) => line.startsWith(">") && line !== ">" && !line.startsWith("> "))) {
      fail(`${rel(file)}: chapter ending block quote lines should use ">" or "> " markers`);
    }

    const beforeEndingHeader = previousNonEmptyLine(lines, endingHeaderIndex);
    if (!beforeEndingHeader || beforeEndingHeader.line !== "---") {
      fail(`${rel(file)}: chapter ending block should be separated from body by a horizontal rule`);
    }
    ...
    if (lines[endingHeaderIndex + 1] !== ">") {
      fail(`${rel(file)}: chapter ending title should be followed by a blank blockquote line`);
    }

    const endingBlockquoteLines = endingBodyLines.filter((line) => line.startsWith(">"));
    if (endingBlockquoteLines.length !== 3) {
      fail(`${rel(file)}: chapter ending block should contain exactly title, blank spacer, and one summary line`);
    }

    if (endingDividerIndex !== endingHeaderIndex + 4 || lines[endingHeaderIndex + 3] !== "") {
      fail(`${rel(file)}: chapter ending block should be followed by exactly one blank line before the bottom divider`);
    }
...
function checkChapterTitleBlocks() {
  for (const chapter of chapters) {
    ...
    const visibleLines = stripFencedBlocks(read(file)).split(/\r?\n/);
    const headingCount = visibleLines.filter((line) => line === `# ${heading}`).length;
    const subtitleCount = visibleLines.filter((line) => line === `## ${subtitle}`).length;

    if (headingCount !== 1) {
      fail(`${rel(file)}: chapter title heading should appear exactly once`);
    }

    if (subtitleCount !== 1) {
      fail(`${rel(file)}: chapter subtitle heading should appear exactly once`);
    }
  }
}
...
function checkLocalLinks(file, options = {}) {
  const text = stripFencedBlocks(read(file));
  const linkPattern = /\[[^\]]+\]\(([^)]+)\)/g;
  const sourceIsManagedMarkdown = isInsideDirectory(projectRoot, file);
  const boundaryRoot = sourceIsManagedMarkdown ? projectRoot : options.boundaryRoot;
  const boundaryLabel = sourceIsManagedMarkdown ? "agent-murim" : options.boundaryLabel;
  let match;
  ...
  while ((match = linkPattern.exec(text)) !== null) {
    ...
    if (boundaryRoot && !isInsideDirectory(boundaryRoot, resolved)) {
      fail(`${rel(file)}: local link target should stay inside ${boundaryLabel} ${target}`);
      continue;
    }
...
function checkSeriesReadmeSectionOrder() {
  const readmePath = path.join(projectRoot, "README.md");
  ...
  const actualHeadings = stripFencedBlocks(read(readmePath))
    .split(/\r?\n/)
    .filter((line) => line.startsWith("## "))
    .join("\n");

  if (actualHeadings !== expectedHeadings) {
    fail(`${rel(readmePath)}: H2 section order should match canonical series README sections without missing, duplicate, or extra headings`);
  }
}
...
function checkDistributionReadmeSectionOrder() {
  const distReadmePath = path.join(projectRoot, "dist", "README.md");
  ...
  const actualHeadings = stripFencedBlocks(read(distReadmePath))
    .split(/\r?\n/)
    .filter((line) => line.startsWith("## "))
    .join("\n");

  if (actualHeadings !== expectedHeadings) {
    fail(`${rel(distReadmePath)}: H2 section order should match canonical distribution README sections without missing, duplicate, or extra headings`);
  }
}
...
function checkDistributionReadmeArchiveTable() {
  ...
  const actualSection = readme.slice(tableStart, nextHeadingStart).trimEnd();

  if (actualSection !== expectedSection) {
    fail(`${rel(distReadmePath)}: archive table section should match canonical distribution package table without missing, duplicate, or extra lines`);
  }
}
...
function checkLayoutCommonPrinciplesSection() {
  ...
  const expectedSection = [
    ...
    "- 상단 페이지네이션은 빈 줄, `---`, 빈 줄 순서로 본문과 분리한다.",
    "- 하단 페이지네이션은 직전 `---`와 정확히 한 줄의 빈 줄로 분리한다.",
    "- LAYOUT 제목/소개 구획도 정해진 문장만 중복 없이 유지한다.",
    "- LAYOUT 공통 원칙 섹션도 정해진 문장과 목록만 중복 없이 유지한다.",
    "- LAYOUT 파일명 규칙 섹션도 정해진 문장과 목록만 중복 없이 유지한다.",
    "- LAYOUT 첫 장과 최신 장 규칙 섹션도 정해진 문장과 목록만 중복 없이 유지한다.",
    "- LAYOUT 용어 표기 섹션도 정해진 표만 중복 없이 유지한다.",
    ...
    "- 루트 README 로컬 링크도 실제 파일과 앵커를 가리키며 repo 밖을 벗어나지 않도록 유지한다.",
    ...
    "- 배포본 README 섹션 순서도 정해진 heading 목록을 중복 없이 유지한다.",
    "- 배포본 README 압축 파일 표 섹션도 정해진 표만 중복 없이 유지한다.",
    ...
    "- 본문 파일의 canonical 장 제목과 장 부제 heading은 각각 1번만 등장해야 한다.",
    "- 각 본문 파일에는 장 종료 안내 블록을 정확히 1개만 두고, 하단 페이지네이션 직전 `---` 바로 위에 배치한다.",
    "- 장 종료 안내 블록 바로 앞에는 본문과 분리하는 `---`를 둔다.",
    "- 장 종료 안내 블록의 인용문은 빈 줄 `>` 또는 공백 있는 `> 문장` 형식만 사용한다.",
    "- 장 종료 안내 제목은 `프롤로그 종료`, `1화 종료`, `에필로그 종료`처럼 해당 장 번호/구분과 정확히 일치해야 한다.",
    "- 장 종료 안내 제목 바로 다음 줄은 빈 인용문 `>`로 두어 제목과 요약을 분리한다.",
    "- 장 종료 안내 블록은 제목, 빈 인용문, 요약 인용문 총 3줄의 blockquote로 구성한다.",
    "- 장 종료 안내 블록 뒤에는 하단 분리선 전 빈 줄을 정확히 1줄만 둔다.",
  ...
  const actualSection = layout.slice(sectionStart + sectionHeading.length, nextHeadingStart).trimEnd();

  if (actualSection !== expectedSection) {
    fail(`${rel(layoutPath)}: common principles section should match canonical layout rules without missing, duplicate, or extra lines`);
  }
}
...
function checkLayoutFilenameRulesSection() {
  ...
  if (actualSection !== expectedSection) {
    fail(`${rel(layoutPath)}: filename rules section should match canonical manuscript file naming without missing, duplicate, or extra lines`);
  }
}
...
function checkLayoutFirstAndLatestRulesSection() {
  ...
  if (actualSection !== expectedSection) {
    fail(`${rel(layoutPath)}: first/latest rules section should match canonical chapter-edge navigation without missing, duplicate, or extra lines`);
  }
}
...
function checkLayoutTerminologySection() {
  ...
  if (actualSection !== expectedSection) {
    fail(`${rel(layoutPath)}: terminology section should match canonical AI-murim terms without missing, duplicate, or extra lines`);
  }
}
...
function checkRootReadmeLocalLinks() {
  const rootReadmePath = path.join(repoRoot, "README.md");
  ...
  checkLocalLinks(rootReadmePath, { boundaryRoot: repoRoot, boundaryLabel: "repository" });
}

function checkRootReadmeMarkdownHygiene() {
  const rootReadmePath = path.join(repoRoot, "README.md");
  ...
  checkCodeFences(rootReadmePath);
  checkMarkdownWhitespace(rootReadmePath);
}
...
function checkRootReadmeOneLineIntroSection() {
  const rootReadmePath = path.join(repoRoot, "README.md");
  ...
  if (actualSection !== expectedSection) {
    fail(`${rel(rootReadmePath)}: root one-line intro section should match canonical work summaries without missing, duplicate, or extra lines`);
  }
}
...
checkDistributionReadmeSectionOrder();
...
checkRootReadmeLocalLinks();
checkRootReadmeMarkdownHygiene();
